...
describe('/withdraw/eth', () => {
  it('should exist', async () => {
    const response = await supertest(app)
      .post(`/${constants.BASE_API_PATH}/withdraw/eth`)
  ...
  it('should send eth whith prvKey from env', async () => {
    ...
    expect(callArgs[0]).to.eq(xprv)
    expect(callArgs[1]).to.eq('123')
    expect(callArgs[2]).to.eq('100')
    expect(callArgs[3]).to.eq('')
  })
  ...
  it('should send eth with prvKey from params', async () => {
    ...
    expect(data).to.eq('eth')
    expect(callArgs[0]).to.eq(xprv)
    expect(callArgs[1]).to.eq('1234')
    expect(callArgs[2]).to.eq('1000')
    expect(callArgs[3]).to.eq('aa')
  })
